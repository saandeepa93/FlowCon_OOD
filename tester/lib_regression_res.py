...
def block_split_RealNVP(X, Y, out):
    ...
    return X_train, Y_train, X_test, Y_test, partition
...
def detection_performance(regressor, X, Y, outf):
    ...
    results_ours = get_metrics_ood(Y, y_pred, invert_score=False)
...
def load_characteristics_RealNVP(score, dataset, out, outf, net_type, num_classes):
    ...
    if net_type == 'wideresnet':
        ...
        X = np.stack((score_L1, score_L2, score_L3), axis=-1)
        # X = score_L3.reshape(-1, 1)
        ...
        X = np.stack((score_L1, score_L2, score_L3, score_L4),axis=-1)
        # X = score_L4.reshape(-1, 1)
